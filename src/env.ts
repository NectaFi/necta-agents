...
export const envSchema = z
	...
	.refine(
		(data) => {
			// If agents are enabled, BRAHMA_ACCOUNT_ADDRESS must be set
			if (data.ENABLE_AGENTS && !data.BRAHMA_ACCOUNT_ADDRESS) {
				throw new Error('BRAHMA_ACCOUNT_ADDRESS is required when ENABLE_AGENTS is true')
			}
			return true
		},
		{
			message: 'BRAHMA_ACCOUNT_ADDRESS is required when ENABLE_AGENTS is true',
		}
	)
